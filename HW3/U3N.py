...
# ======== 2. U-Net 3-blocks (with BN) ========
class UNet_3block(nn.Module):
...
class RetinaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask = (np.array(mask) / 255.0 > 0.5).astype(np.float32)
...
def train_model():
    ...
    for epoch in range(1, 31):
        ...
        for step, (img, mask) in enumerate(train_loader, 1):
            ...
            if step % 10 == 0:
                print(f"Step {step:04d}/{len(train_loader)} | Loss = {loss.item():.4f}")

        with torch.no_grad():
            dice = dice_coef(pred, mask).item()
        avg_loss = total_loss / len(train_loader)
        print(f"Epoch {epoch}: Avg Loss = {avg_loss:.4f}, Dice = {dice:.4f}")

        if dice > best_dice:
            best_dice = dice
            torch.save(model.state_dict(), "best_unet3_withBN.pth")
...
# ======== 6. Test ========
def test_model(model_path="best_unet3_withBN.pth"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = UNet_3block().to(device)
    ...
    with torch.no_grad():
        for i, (img, mask) in enumerate(test_loader):
            ...
            ious.append(iou)
            if i < 5:
                print(f"Image {i+1:03d}: Dice = {dice:.4f}, IoU = {iou:.4f}")
...
if __name__ == "__main__":
    train_model()
    # After training:
    # test_model("best_unet3_withBN.pth")
