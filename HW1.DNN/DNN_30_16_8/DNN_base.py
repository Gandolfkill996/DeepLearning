...
# ============ Training ============
# def train_model(model, train_loader, val_loader, optimizer, criterion, epochs=20, patience=5):
...
#             # 检查 loss 是否 NaN
#             if torch.isnan(loss):
#                 print(f"⚠️ Loss NaN at Epoch {epoch+1}, Batch {batch_idx+1}, skipping update")
#                 continue
#
#             loss.backward()
...
def train_model(model, train_loader, val_loader, optimizer, criterion, epochs=20, patience=5, device="cpu"):
    ...
    for epoch in range(epochs):
        ...
        for Xb, yb in train_loader:
            ...
            if torch.isnan(loss):
                print(f"⚠️ Loss NaN at Epoch {epoch+1}, skipping batch")
                continue
...
# ============ Grid Search ============
def run_gridsearch(ModelClass, model_name="DNN16", device="cpu"):
    ...
    for lr in learning_rates:
        ...
        for dropout, epochs, batch in itertools.product(dropout_probs, epochs_list, batch_sizes):
            ...
            history = train_model(model, train_loader, val_loader, optimizer, criterion, epochs, patience=5, device=device)
...
# ============ Test Model ============
def test_model(ModelClass, model_name="DNN_30_16_8", new_data_path="cancer_reg-1.csv", model_path=None, params_path=None):
    import pandas as pd
    path = new_data_path
    data = Data(path)
    X, y = data.preprocess()
