...
To train the VGG models at different learning rates (0.1, 0.01, 0.001),
...
During training, the script will:
  • Train three VGG models with different learning rates.
  • Save the best model (based on validation accuracy) to:
        outputs/lr_<learning_rate>/best_vgg_lr<learning_rate>.pth
...
After training, you can evaluate the best VGG model on **10% of the MNIST test dataset**
...
Then in Terminal (interactive Python mode or inside the main block):
    ...
    >>> from VGG import test_model
    >>> import torch
    >>> device = torch.device("cuda" if torch.cuda.is_available() else
    ...                       "mps" if torch.backends.mps.is_available() else "cpu")
    >>> test_model("outputs/lr_0.001/best_vgg_lr0.001.pth", device)

Or simply run the test block already included in VGG.py:
    python VGG.py --test
...
This function will:
  ...
  • Print metrics to Terminal, e.g.:
        ✅ Test Results (lr=0.001) -> Accuracy: 0.9751, F1: 0.9748, AUC: 0.9988
...
Each learning rate folder (lr_0.1, lr_0.01, lr_0.001) contains:
  - best_vgg_lr*.pth           → Saved best model weights.
...
• You can modify batch size, epochs, or learning rates inside VGG.py for experiments.