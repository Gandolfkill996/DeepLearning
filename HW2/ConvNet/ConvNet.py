...
class ConvNet(nn.Module):
    ...
    def forward(self, x):
        x1 = torch.relu(self.conv1(x))     # First conv layer feature
        x2 = torch.relu(self.conv2(x1))    # Second conv layer feature
        x = self.pool(x2)
        x = torch.flatten(x, 1)
...
def visualize_features(model, test_loader, save_dir, device='cpu'):
    ...
    # visualize the first layer's 16 channels (mean)
    plt.figure(figsize=(6, 4))
    plt.imshow(x1[0, :16].mean(axis=0), cmap='viridis')
    plt.title("First Layer Feature Map (mean of 16 channels)")
    plt.colorbar()
    ...
    plt.title("Second Layer Feature Map (mean of 16 channels)")
    plt.colorbar()
...
def test_model(model_path, device, output_dir="outputs/test_eval"):
    """
    Load the trained ConvNet model and evaluate on 10% of the MNIST test dataset.
    ...
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.title("ConvNet ROC Curve (Class 0 vs Rest)")
...
if __name__ == "__main__":
    ...
    for lr in learning_rates:
        print(f"\n===== Training ConvNet with learning rate = {lr} =====")
        ...
        # Save model
        model_path = f"{lr_dir}/best_convnet_lr{lr}.pth"
